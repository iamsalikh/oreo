...
def home_page(request):
    search_bar = forms.SearchFrom()
    #Собираем название всех продуктов
    product_info = Product.objects.all()
    #Собираем названия всез категорий
...
def product_page(request, pk):
    ...
    if request.method == 'POST':
        Cart.objects.create(user_id=request.user_id,
...
def get_user_cart(request):
    ...
    if request.method == 'POST':
        ...
        for i in user_cart:
            main_text += f'Товап: {i.user_product}\nКоличество: {i.user_product_amount}'

        bot.send_message(6149766362, main_text)
        user_cart.delete()
        return redirect('/')
